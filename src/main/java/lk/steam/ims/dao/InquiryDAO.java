...
public interface InquiryDAO extends JpaRepository<Inquiry,Integer> {
    ...
    @Query(value = "Select * from inquiry where date(timestamp)>=?1 and date(timestamp)<=?2 and course_id=?3 and addedby=?4",nativeQuery = true)
    List<Inquiry> getInquiriesByDateRangeAndCourseForLoggedInUser(String startDate, String endDate, Integer courseId,String addedBy);
    ...
    @Query(value = "SELECT i from Inquiry i where i.id=?1")
    Inquiry getInquiryNumberById(Integer inquiryID);

    @Query(value = "select * from inquiry where date(timestamp)>=?1 and date(timestamp)<=?2 and source_id=?3 and course_id=?4 and addedby=?5 and (inquirynumber =?6 or primarymobilenumber=?6 or idvalue=?6)",nativeQuery = true)
