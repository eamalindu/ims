window.addEventListener('load',()=> {
    ...
    refreshReportDroppedWithOutFollowUpsTable(startDate,endDate);
...
const refreshReportDroppedWithOutFollowUpsTable = (startDate,endDate) =>{
     ...
     droppedInquiriesWithOutInquiries = ajaxGetRequest("/Inquiry/getAllInquiriesByDateRangeAndStatus/"+startDate+"/"+endDate+"/"+4);
...
const resetReportSearchBar = ()=>{
    ...
    function cb(start, end) {
        $('#droppedFollowupInquirySearchDateRange span').html(start.format('YYYY-MMMM-DD') + ' - ' + end.format('YYYY-MMMM-DD'));
    }

    $('#droppedFollowupInquirySearchDateRange').daterangepicker({
...
const getReport = ()=>{
    const [startDate,endDate] = droppedFollowupInquirySearchDateRange.value.split(' - ');
    refreshReportDroppedWithOutFollowUpsTable(startDate,endDate);
}

const exportData = ()=>{
    showCustomConfirm('You are about to export <span class="text-purple">Dropped Inquires Without FollowUps</span> data to an Excel spreadsheet<br><br>Are You Sure?',function (result){
        if(result){
            exportToExcel(droppedInquiriesWithOutInquiries,'Inquiries Dropped Without Followup Report',reportColumnFormat);
        }
    });
}